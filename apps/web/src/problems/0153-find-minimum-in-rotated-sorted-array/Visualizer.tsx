...
import type { FindMinimumRotatedExample } from "./data";
import { createFindMinimumRotatedDryRun } from "./dryRun";

export default function FindMinimumRotatedArrayVisualizer({ onBack }: VisualizerProps) {
  ...
  const dryRun = useMemo(() => createFindMinimumRotatedDryRun(nums), [nums]);
  ...
  function loadExample(example: FindMinimumRotatedExample) {
  ...
  function loadInput() {
    try {
      ...
      if (
        ...
        setError("Use a JSON array of 1 to 14 integers.");
        return;
      }
      ...
      setError("Use valid JSON, for example [3,4,5,1,2].");
    }
  }
    ...
    <main className="app-shell">
      ...
      <section className="workspace">
        <aside className="board-panel">
          ...
          <div className="panel-heading"><h2>input</h2><span>len = {nums.length}</span></div>
          <div className="expected-output"><span>candidate window</span><code>[{frame.left}, {frame.right}]</code></div>
        ...
        <section className="flow-panel binary-flow-panel">
          <div className="panel-heading"><h2>Rotated Minimum Search</h2><span>mid: {frame.mid ?? "none"}</span></div>
          <div className="binary-stage">
            <div className="binary-array">
              {frame.nums.map((value, index) => {
                ...
                const inCandidate = frame.candidateRange ? index >= frame.candidateRange[0] && index <= frame.candidateRange[1] : false;
                const isDiscarded = frame.discardedRange ? index >= frame.discardedRange[0] && index <= frame.discardedRange[1] : false;
                return (
                  <div
                    className={[
                      "binary-cell",
                      inWindow ? "in-window" : "",
                      inCandidate ? "is-target-range" : "",
                      isDiscarded ? "is-discarded" : "",
                      frame.left === index ? "is-left" : "",
                      frame.mid === index ? "is-mid" : "",
                      frame.right === index ? "is-right" : "",
                      frame.resultIndex === index ? "is-found" : "",
                    ].filter(Boolean).join(" ")}
                    key={`${value}-${index}`}
                  >
                    <strong>{value}</strong>
        ...
        <aside className="state-panel">
          ...
          <div className="state-block"><h3>state</h3><div className="token-list"><span>left = {frame.left}</span><span>mid = {frame.mid ?? "none"}</span><span>right = {frame.right}</span><span>result = {frame.result ?? "pending"}</span></div></div>
          <div className="state-block"><h3>decision</h3><div className="token-list"><span>minimum side = {frame.minimumSide ?? "unknown"}</span><span>candidate = [{frame.candidateRange?.[0] ?? "none"}, {frame.candidateRange?.[1] ?? "none"}]</span></div></div>
