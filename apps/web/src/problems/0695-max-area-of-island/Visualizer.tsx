...
import { codeLines, defaultExample, examples, title, type MaxAreaOfIslandInput } from "./data";
import { createMaxAreaOfIslandDryRun } from "./dryRun";

const keyOf = (cell: Cell | null) => cell ? `${cell[0]}-${cell[1]}` : "";

export default function MaxAreaOfIslandVisualizer({ onBack }: VisualizerProps) {
  const [selectedExampleId, setSelectedExampleId] = useState<number>(defaultExample.id);
  const [input, setInput] = useState<MaxAreaOfIslandInput>(defaultExample.input);
  const [gridInput, setGridInput] = useState(JSON.stringify(defaultExample.input.grid));
  const [step, setStep] = useState(0);
  const [playing, setPlaying] = useState(false);
  const [error, setError] = useState("");
  const selectedExample = examples.find((example) => example.id === selectedExampleId);
  const dryRun = useMemo(() => createMaxAreaOfIslandDryRun(input.grid), [input]);
  const frame = dryRun.frames[Math.min(step, dryRun.frames.length - 1)]!;
  const cols = input.grid[0]?.length ?? 1;

  useEffect(() => { if (!playing || step >= dryRun.frames.length - 1) { if (step >= dryRun.frames.length - 1) setPlaying(false); return; } const timer = window.setTimeout(() => setStep((current) => current + 1), 500); return () => window.clearTimeout(timer); }, [dryRun.frames.length, playing, step]);
  ...
  function loadInput() { try { const grid = JSON.parse(gridInput) as number[][]; if (!isBinaryGrid(grid)) throw new Error(); setSelectedExampleId(0); setInput({ grid }); setStep(0); setPlaying(false); setError(""); } catch { setError("Use a rectangular 0 / 1 matrix up to 8 x 13."); } }
