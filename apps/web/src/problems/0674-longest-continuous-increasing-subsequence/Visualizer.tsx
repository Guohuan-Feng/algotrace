...
import type { LcisExample } from "./data";
import { createLcisDryRun } from "./dryRun";

export default function LongestContinuousIncreasingSubsequenceVisualizer({ onBack }: VisualizerProps) {
  ...
  const dryRun = useMemo(() => createLcisDryRun(nums), [nums]);
  ...
  function loadExample(example: LcisExample) {
  ...
  function loadInput() {
    try {
      ...
      if (!Array.isArray(parsed) || parsed.length > 14 || !parsed.every(Number.isInteger)) { setError("Use a JSON array of at most 14 integers."); return; }
      setSelectedExampleId(0); setNums(parsed); setStep(0); setPlaying(false); setError("");
    } catch { setError("Use valid JSON, for example [1,3,5,4,7]."); }
  }
  ...
  return (
    <main className="app-shell">
      ...
      <section className="workspace">
        ...
        <section className="flow-panel sequence-flow-panel"><div className="panel-heading"><h2>LCIS DP</h2><span>result: {frame.result ?? Math.max(0, ...frame.dp)}</span></div><DpStrip nums={frame.nums} dp={frame.dp} i={frame.i} previous={frame.previous} /></section>
        <aside className="state-panel">
          ...
          <div className="state-block"><h3>state</h3><div className="token-list"><span>i = {frame.i ?? "none"}</span><span>prev = {frame.previous ?? "none"}</span><span>max dp = {Math.max(0, ...frame.dp)}</span></div></div>
...
function DpStrip({ nums, dp, i, previous }: { nums: number[]; dp: number[]; i: number | null; previous: number | null }) {
  return <div className="sequence-stage"><div className="dp-strip">{nums.length ? nums.map((value, index) => <div className={["dp-card", i === index ? "is-current" : "", previous === index ? "is-previous" : ""].filter(Boolean).join(" ")} key={index}><strong>{value}</strong><span>i={index}</span><em>dp={dp[index] ?? "-"}</em></div>) : <p className="heap-empty">empty nums</p>}</div></div>;
}
