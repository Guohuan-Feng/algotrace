...
import type { PermutationsExample } from "./data";
import { createPermutationsDryRun, nodeIdForPath } from "./dryRun";

export default function PermutationsVisualizer({ onBack }: VisualizerProps) {
  ...
  const dryRun = useMemo(() => createPermutationsDryRun(nums), [nums]);
  const frame = dryRun.frames[Math.min(step, dryRun.frames.length - 1)];
  const layout = useMemo(() => layoutPermutationTree(nums), [nums]);
  ...
  const resetTreeViewport = useCallback((duration = 0) => {
    ...
    const zoom = nums.length >= 5 ? 0.28 : nums.length >= 4 ? 0.46 : nums.length >= 3 ? 0.82 : 1;
  ...
  function loadInput() {
    try {
      ...
      if (!Array.isArray(parsed) || parsed.length < 1 || parsed.length > 5 || !parsed.every(Number.isInteger)) {
        setError("Use a JSON array of 1 to 5 integers, for example [1,2,3].");
        return;
      }

      if (new Set(parsed).size !== parsed.length) {
        setError("LeetCode 46 uses distinct integers, so every number must be unique.");
        return;
      }
      ...
      setError("Use valid JSON, for example [1,2,3].");
    }
  }
      ...
      <section className="workspace">
        <aside className="board-panel">
          ...
          <div className="digits-strip number-strip" aria-label="Candidate numbers">
            {nums.map((value) => (
              <span
                className={[
                  frame.currentNum === value ? "active" : "",
                  frame.path.includes(value) ? "used" : "",
                  frame.skippedNum === value ? "skipped" : "",
                ]
                  .filter(Boolean)
                  .join(" ")}
                key={value}
        ...
        <aside className="state-panel">
          ...
          <div className="state-block">
            <h3>current state</h3>
            <div className="token-list">
              <span>nums = [{nums.join(", ")}]</span>
              <span>path = [{frame.path.join(", ")}]</span>
              {frame.currentNum !== null ? <span>num = {frame.currentNum}</span> : null}
            </div>
          </div>
...
function layoutPermutationTree(nums: number[]): Map<string, { x: number; y: number }> {
  const positions = new Map<string, { x: number; y: number }>();
  const horizontalGap = nums.length >= 5 ? 38 : nums.length >= 4 ? 62 : nums.length >= 3 ? 118 : 150;
  const verticalGap = 116;
  let nextLeaf = 0;

  function measure(path: number[]): number {
    ...
    for (const value of nums) {
      if (!path.includes(value)) {
        childXs.push(measure([...path, value]));
      }
    }
