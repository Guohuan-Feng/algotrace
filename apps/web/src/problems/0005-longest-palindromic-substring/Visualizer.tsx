...
import type { LongestPalindromicSubstringExample } from "./data";
import { createLongestPalindromicSubstringDryRun } from "./dryRun";

export default function LongestPalindromicSubstringVisualizer({ onBack }: VisualizerProps) {
  ...
  const dryRun = useMemo(() => createLongestPalindromicSubstringDryRun(s), [s]);
  ...
  function loadExample(example: LongestPalindromicSubstringExample) {
  ...
  const modeLabel = frame.mode ? `${frame.mode} center` : "compare candidates";
    ...
    <main className="app-shell">
      ...
      <section className="workspace">
        <aside className="board-panel">
          ...
          <div className="input-grid"><label>s<input className="text-input" value={sInput} onChange={(event) => setSInput(event.target.value)} /></label>{error ? <p className="error">{error}</p> : null}<button className="command load" onClick={loadInput} type="button"><Upload size={16} />Load string</button></div>
          ...
          {selectedExample?.note ? <p className="center-example-note">{selectedExample.note}</p> : null}
        ...
        <section className="flow-panel center-expansion-flow-panel">
          ...
          <div className="center-expansion-stage">
            <div className="center-mode-row" aria-label="center type"><div className={frame.mode === "odd" ? "center-mode is-selected" : "center-mode"}><strong>odd</strong><span>expand(i, i)</span></div><div className={frame.mode === "even" ? "center-mode is-selected" : "center-mode"}><strong>even</strong><span>expand(i, i + 1)</span></div></div>
            <div className="center-string-scroll"><div className="center-string" aria-label={`string ${s}`}>{s.split("").map((char, index) => <StringCell key={index} char={char} index={index} frame={frame} />)}</div></div>
            <div className={`center-range-caption ${frame.comparison ?? ""}`}><span>l = {frame.l ?? "-"}</span><strong>{comparisonLabel(frame.comparison)}</strong><span>r = {frame.r ?? "-"}</span></div>
            <div className="longest-readout"><div><span>s1</span><strong>{frame.s1 || '""'}</strong></div><div><span>s2</span><strong>{frame.s2 || '""'}</strong></div><div className="is-best"><span>res</span><strong>{frame.res || '""'}</strong></div></div>
          </div>
        </section>
        ...
        <aside className="state-panel">
          ...
          <div className="state-block"><h3>candidates</h3><div className="token-list words"><span>s1 = {frame.s1 || '""'}</span><span>s2 = {frame.s2 || '""'}</span><span>res = {frame.res || '""'}</span></div></div>
...
function StringCell({ char, index, frame }: { char: string; index: number; frame: ReturnType<typeof createLongestPalindromicSubstringDryRun>["frames"][number] }) {
...
function comparisonLabel(comparison: ReturnType<typeof createLongestPalindromicSubstringDryRun>["frames"][number]["comparison"]) {
